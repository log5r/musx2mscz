...
class StyleBuilder:
    ...
    def _page_and_spatium(self):
        ...
        pct = 100.0
        if page is not None:
            h = doc.get(page, "height")
            w = doc.get(page, "width")
            pct = float(doc.get(page, "percent", "100") or 100)
            if h and w:
                w_in, h_in = int(w) / EVPU_PER_INCH, int(h) / EVPU_PER_INCH
                self.values["pageWidth"] = f"{w_in:.3f}"
                self.values["pageHeight"] = f"{h_in:.3f}"
                # margins (Finale stores top/right sign-reversed)
                ...
                self.values["pagePrintableWidth"] = f"{w_in - m_left - m_right:.3f}"
                self.values["pagePrintableHeight"] = f"{h_in - m_top - m_bottom:.3f}"
                for k, v in (("pageOddLeftMargin", m_left), ("pageEvenLeftMargin", m_left),
                             ("pageOddTopMargin", m_top), ("pageEvenTopMargin", m_top),
                             ("pageOddBottomMargin", m_bottom), ("pageEvenBottomMargin", m_bottom)):
                    self.values[k] = f"{v:.3f}"
                self.values["pageTwosided"] = "1"

        # Effective staff scaling (see musxdom StaffSystem::calcSystemScaling):
        #   ssysPercent/100 * staffHeight / (4 spaces * 1536 Efix per space)
        # multiplied by the page percent. Base space is 24 EVPU.
        sysspec = doc.other("staffSystemSpec", "1")
        sys_pct = 100.0
        staff_height_frac = 1.0  # staffHeight / 6144
        ...
        spatium_mm = space_evpu * MM_PER_INCH / EVPU_PER_INCH
        spatium_mm = max(0.4, min(3.0, spatium_mm))
        self.values["spatium"] = f"{spatium_mm:.5g}"  # millimeters
        ...
        if len(dists) > 1:
            ...
            # Finale positions staves on a fixed grid; disable MuseScore's
            # collision-driven vertical spreading so systems keep Finale's height.
            self.values["enableVerticalSpread"] = "0"
            self.values["minVerticalDistance"] = "0.3"
    ...
    def _category_text_styles(self):
        for cat in self.doc.other_all("markingsCategory"):
            ...
            for prefix in prefixes:
                ...
                if font.size:
                    self.values[f"{prefix}FontSize"] = f"{font.size:g}"
    ...
    def _misc_fonts(self):
        ...
        if tb.family and not is_music_font(tb.family):
            ...
            if tb.size:
                self.values["defaultFontSize"] = f"{tb.size:g}"
        ...
        if lyr.family and not is_music_font(lyr.family):
            for prefix in ("lyricsOdd", "lyricsEven"):
                ...
                if lyr.size:
                    self.values[f"{prefix}FontSize"] = f"{lyr.size:g}"
        ...
        if tup.family and not is_music_font(tup.family):
            ...
            if tup.size:
                self.values["tupletFontSize"] = f"{tup.size:g}"
